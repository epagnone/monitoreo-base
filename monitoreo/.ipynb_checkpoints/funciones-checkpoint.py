...
class funciones:
    ...
    def vdi(id, fecha_foto, variables, abt_modelo, nombre_modelo, tipo_variables='cualitativas', ambiente='sdb_datamining'):
        ...
        df_baseline = sqlContext.sql(query)
    ...
    def vdi_cualitativas(id, score, variables,fecha_foto, abt_modelo, nombre_modelo, ambiente='sdb_datamining' ):
        ...
        # Validamos que se ingrese la cantidad de variables adecuada en base al parámetro cant_vars
        ...
        """
        df_vdi_cuali_bl= sqlContext.sql(query)
    ...
    def vdi_cuantitativas(id, score, variables,fecha_foto, abt_modelo, nombre_modelo, ambiente='sdb_datamining'):
        ...
        # Validamos que se pasen como parámetro solamente 10 variables
        assert len(variables)==10, 'Tiene que insertar solamente 10 variables'  
        ...
        df_actual_ = sqlContext.sql(query)
        ...
        and var in ({lista_variables})
        """
        df_vdi_cuanti_bl= sqlContext.sql(query)
        ...
        #result = sqlContext.sql("""set hive.exec.dynamic.partition.mode=nonstrict""")
        #df_vdi_cuanti_move.write.mode("overwrite").insertInto(f"{ambiente}.indicadores_vdi",overwrite=True)
