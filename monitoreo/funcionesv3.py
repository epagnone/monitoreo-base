...
class Funciones:
    ...
    def performance(self, query, fecha_score, cant_bines = 20):
    ...
    def psi(score, fecha_foto):
    
    def insert_psi(df_psi):
    
    def vdi_cualitativas( score, variables,fecha_foto):
    
    def vdi_cuantitativas(score, variables,fecha_foto,):
    ...
    def calcular_performance_actual(self, query, fecha_score, cant_bines = 20, ambiente='sdb_datamining'):
        """
        Función que calcula las métricas de performance (para modelos de clasificación binaria) tales como: 
        Information Value, Kolmogorov Smirnov (KS), Gini, AUC, para un periodo en particular, considerando un baseline.
        También, crea la tabla de performance (para modelos de clasificación binaria) para un periodo en particular, 
        considerando un baseline, en donde podemos apreciar métricas tales como Lift Acumulado, % Captura, % Conversión.
...
# #Idea de la version 2
#     def performance(self, query:str, fecha_score, cant_bines = 20):
#         self.lista_df.append(__performance)
#         return self.__performance(query, fecha_score, cant_bines, tipo="PERFORMANCE")
    ...
    #         df_vdi_bl['modelo']= self.nombre_modelo
    ...
    # def insert_vdi_bl(self, df_vdi_bl, tipo_variables='cualitativas') -> None: #Podria devolver error o hacer un raise en caso de fallo
    ...
    #     if flag:
    #         df_vdi_bl.write.mode("overwrite").insertInto(f"{self.ambiente}.{table}",overwrite=True)   
    ...
    # def __performance(self, query, fecha_score, cant_bines = 20, tipo= 'PERFORMANCE'): #ToDo: tipo no es redundante? al final la funcion es para performance
    #     """
    ...
    #         # Cargamos el último baseline y lo pasamos a pandas
    #         query=f"""
    #         select max_p_target from {self.ambiente}.indicadores_bines 
    #         where modelo='{self.nombre_modelo}' and tipo='BASELINE' 
    #         and fecha =(select max(fecha) from {self.ambiente}.indicadores_bines 
    #                 where modelo='{self.nombre_modelo}' and tipo='BASELINE' )
    #         order by max_p_target"""
    #         df_baseline=spark.sql(query)
    #         df_baseline=df_baseline.toPandas()
    ...
    # def __insert_performance(self, df_res_bines, df_res_metricas, df_psi=0, tipo = 'PERFORMANCE'): #ToDo: idem anterior
    ...
    #     # Insertamos la tabla de performance 
    #     df_res_bines.write.mode("overwrite").insertInto(f"{self.ambiente}.indicadores_bines",overwrite=True)
        
    #     # Insertamos la tabla de métricas de performance
    #     df_res_metricas.write.mode("overwrite").insertInto(f"{self.ambiente}.indicadores_performance",overwrite=True)
        
    #     # Insertarmos la tabla de baseline de PSI solamente si se especifica que estamos trabajando con el periodo de BASELINE
    #     if tipo.lower() == 'baseline':
    #         df_psi.write.mode("overwrite").insertInto(f"{self.ambiente}.indicadores_psi_bl",overwrite=True)
    #         print(f'Insertamos {df_res_bines.count()} registros en la tabla indicadores_bines, {df_res_metricas.count()} en la tabla indicadores_performance y {df_psi.count()} en la tabla indicadores_psi_bl')
    #     else:
    #         print(f'Insertamos {df_res_bines.count()} registros en la tabla indicadores_bines, {df_res_metricas.count()} en la tabla indicadores_performance.')
    ...
    #     # Cargamos el periodo actual y lo pasamos a pandas
    #     query =f"""
    #     select {id} as id ,{score} as score from {self.ambiente}.{self.abt_modelo} where periodo={periodo}
    #     """
    #     df_bin_ = spark.sql(query)
    #     df_bin = df_bin_.toPandas()
    ...
    #     query=f"""
    #     select fecha, bin, max_p_target, totales as baseline_counts 
    #     from {self.ambiente}.indicadores_psi_bl
    #     where modelo='{self.nombre_modelo}'
    #     and fecha = (select max(fecha) from {self.ambiente}.indicadores_psi_bl where  modelo='{self.nombre_modelo}') 
    #     order by bin
    #     """
    #     df_baseline_psi_= spark.sql(query)
    #     df_baseline_psi = df_baseline_psi_.toPandas()
    ...
    #     # Insertamos la tabla de psi 
    #     df_psi.write.mode("overwrite").insertInto(f"{self.ambiente}.indicadores_psi",overwrite=True)
    ...
    #     query=f"""
    #     select var, categoria, totales
    #     from {self.ambiente}.indicadores_vdi_cuali_bl
    #     where modelo ='{self.nombre_modelo}' 
    #     and fecha= (select max(fecha) from {self.ambiente}.indicadores_vdi_cuali_bl where modelo ='{self.nombre_modelo}' )
    #     and var in ({lista_variables})
    #     """
    #     df_vdi_cuali_bl= spark.sql(query)
    #     df_vdi_cuali_bl=df_vdi_cuali_bl.toPandas()
    ...
    #     # Levantamos el último baseline de las variables cualitativas
    #     query=f"""
    #     select * from {self.ambiente}.indicadores_vdi_cuanti_bl 
    #     where modelo = '{self.nombre_modelo}'
    #     and fecha = (select max(fecha) from {self.ambiente}.indicadores_vdi_cuanti_bl where  modelo='{self.nombre_modelo}') 
    #     and var in ({lista_variables})
    #     """
    #     df_vdi_cuanti_bl= spark.sql(query)
    ...
    #     # Insertamos la tabla de psi 
    #     df_vdi.write.mode("overwrite").insertInto(f"{self.ambiente}.indicadores_vdi",overwrite=True)
